 ...
 else if($c == 3)
{
   ...
   $stmt= $con->prepare("insert into menue (name,price,size) values (?,?,?)");


if(  $stmt->execute(array($_POST['name'],$_POST['price'],$_POST['size'])) === true )
{ 
...
else if($c == 4)
{
   ...
   $stmt= $con->prepare("update menue set name=?, price=?, size=? where id=?  ");


if(  $stmt->execute(array($_POST['name'],$_POST['price'],$_POST['size'],$_POST['id'])) === true )
{ 
...
{
   echo"<tr>";
   echo "<td>".$row['id']."</td>"."<td>".$row['name']."</td>"."<td>".$row['price']."</td>"."<td>".$row['size']."</td>";
